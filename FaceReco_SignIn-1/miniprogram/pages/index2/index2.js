...
Page({
  ...
  confirm1: function (e) {
    ...
    if ((that.data.sname && that.data.snum)!='') {
      ...
      wx:wx.navigateTo({
        url: '/student_index/student_index?tnam='+e.detail.data.sname+'&snum='+e.detail.data.snum,
  ...
  jump2student_login:function(){
    wx:wx.navigateTo({
      url: '../student_login/student_login',
      success: function(res) {
        console.log("通过API跳转成功")
      },
      // 成功时的回调函数
